...
const en = {
  ...
  autoSent: "Automatically confirm Sent after manual ChatGPT submission",
  autoSentBody: "Applies to new turns. Passively matches the exact submitted prompt and waits for the Zero Risk connector. Never sends or changes the model. If Sent is still available after sending, confirm it yourself before the timer expires.",
...
const zh: Record<keyof typeof en, string> = {
  ...
  browserReady: "浏览器已就绪",
  autoSent: "手动在 ChatGPT 发送后自动确认 Sent",
  autoSentBody: "适用于新轮次。被动匹配完整的已提交提示词，并等待 Zero Risk 连接器启动。不会发送消息或更改模型。如果发送后 Sent 仍可用，请在倒计时结束前手动确认。",
...
const ja: Record<keyof typeof en, string> = {
  ...
  autoSent: "ChatGPT で手動送信した後に Sent を自動確認",
  autoSentBody: "新しいターンに適用されます。送信されたプロンプトの完全一致と Zero Risk コネクターの開始を確認します。送信やモデル変更は行いません。送信後も Sent が表示される場合は、タイマーが切れる前に手動で確認してください。",
...
const ko: Record<keyof typeof en, string> = {
  ...
  autoSent: "ChatGPT에서 직접 전송한 후 Sent 자동 확인",
  autoSentBody: "새 턴에 적용됩니다. 전송된 프롬프트가 정확히 일치하는지 수동 조작 없이 확인하고 Zero Risk 커넥터의 시작을 기다립니다. 메시지를 전송하거나 모델을 변경하지 않습니다. 전송 후에도 Sent가 표시되면 타이머가 끝나기 전에 직접 확인하세요.",
...
const zhTW: Record<keyof typeof en, string> = {
  ...
  browserReady: "瀏覽器已就緒",
  autoSent: "手動在 ChatGPT 傳送後自動確認 Sent",
  autoSentBody: "適用於新回合。被動比對完整的已提交提示詞，並等待 Zero Risk 連接器啟動。不會傳送訊息或變更模型。如果傳送後 Sent 仍可用，請在倒數結束前手動確認。",
